            ...
            for (let i = 0; i < data.length; i++) {
                if (data[i] !== null) {
                    if (data[i] === 'restart') {
                        ...
                        body.cnode(data[i] as Element).up();
                    }
                }
            }
    ...
    static _getRequestStatus(req: Request, def?: number): number {
        ...
        if (req.xhr.readyState === 4) {
            try {
                ...
                log.error(
                    `Caught an error while retrieving a request's status, reqStatus: ${reqStatus}, message: ${(e as Error).message}`,
                );
            }
        }
    ...
    _processRequest(i: number): void {
        ...
        if (req.xhr.readyState === 0) {
            ...
            try {
                ...
                log.error('XHR open failed: ' + (e2 as Error).toString());
