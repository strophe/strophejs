 ...
 * additionally wires up the Node-only transports that must never reach the
 * browser bundle. Right now that is the XEP-0114 external component transport,
 * which pulls in `node:net`/`node:crypto` and the `saxes` stream tokenizer.
 *
